...
function findAllVeggieDishes(dish){
  let results = dishes.filter(function(type) {
    if (type.cuisine === "Vegetarian") {
      return true;
    } else {
      return false;
    }
...
let veggieFood = findAllVeggieDishes();
console.log("Vegetarian Dishes are: ", veggieFood);


//2. Create a function that will return all dishes with the cuisine type of "Italian" and a serving size greater than 5.
//Filter

function findItalianDishesWithFiveServingSizes(dish){
  let results = dishes.filter(function(type) {
    if (type.cuisine === "Italian") {
      if (type.servings >= 5) {
        return true;
      } else{
        return false;
...
let italianFood = findItalianDishesWithFiveServingSizes();
console.log("Italian are: ", italianFood);



//3. Create a function that will return only dishes whose id number matches their serving count.
//Filter
function findAllMatchingIdAndServings(element){
...
let allMatchingIdAndServings = findAllMatchingIdAndServings();
console.log("The dishes that have matching id and servings are: ", allMatchingIdAndServings);


//4. Create a function that will return only dishes whose serving count is even.
//Filter

function findEvenServings(element){
  let results = dishes.filter(function(element) {
...
function findIngredients(dish){
  let results = dishes.filter(function(type) {
    if (type.ingredients.includes("tomato")) {
      if (type.ingredients.includes("cheese")) {
        return true;
      }else {
        return false;
      }
